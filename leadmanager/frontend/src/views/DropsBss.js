...
function ShowPlots(props){
  ...
  if (props.context.state.data){
    ...
    const cellNames = props.context.state.data['DROPS_BSS']['Cell_Names']
    const cellValues = props.context.state.data['DROPS_BSS']['Cell_Values']
    const changePoints = props.context.state.data['DROPS_BSS']['Change_Points']
    ...
    for (var i = 0; i < modifiedDataset.length; i++){
      // the change points are not properly rendered! the dates are always right the colors are most of the times wrong in the svg
      plots.push(
        <div className='my-4'>
          <Row>
            <Col md={9}>
              <LinePlot title={modifiedTitles[i]} date={props.context.state.dates[modifiedChangePoints[i]]} data={modifiedDataset[i]} changePoint={modifiedChangePoints[i]} label={'Drops Bss'} />
            </Col>
            <Col md={3}>
