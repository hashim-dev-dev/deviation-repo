...
function ShowPlots(props){
  ...
  if (props.context.state.data){
    ...
    const cellNames = props.context.state.data['TCH_AVAILABILITY']['Cell_Names']
    const cellValues = props.context.state.data['TCH_AVAILABILITY']['Cell_Values']
    const changePoints = props.context.state.data['TCH_AVAILABILITY']['Change_Points']
    ...
    for (var i = 0; i < modifiedDataset.length; i++){
      plots.push(
        <div className='my-4'>
          <Row>
            <Col md={9}>
              <LinePlot title={modifiedTitles[i]} date={props.context.state.dates[modifiedChangePoints[i]]} data={modifiedDataset[i]} changePoint={modifiedChangePoints[i]} label={'Tch Availability'} />
            </Col>
            <Col md={3}>
